"""Poseidon2 over BN254 (bb blackbox, t = 4) and the option-A Noir hashes (oalib), bit for bit.

Source of truth: research/worldid/prototypes/optA-noir/noir/oalib/src/lib.nr (Sp sponge, leaf_hash,
node_hash, rec tree, code_commitment, half commitment). Permutation constants: HorizenLabs Poseidon2
BN254 t=4 (d=5, 8 full + 56 partial rounds), the same set barretenberg and the Noir blackbox use.
Pure Python ints; field elements are ints in [0, R).
"""
from __future__ import annotations

R = 21888242871839275222246405745257275088548364400416034343698204186575808495617

# internal matrix diagonal minus 1 (M_I = diag(DIAG) + all-ones)
DIAG = (
...
# external round constants, 8 rounds x 4 (first 4 rounds, then last 4)
EXT = (
...
# partial round constants (state[0] only)
INT = (
...
def _m4(a, b, c, d):
    ...
    t4 = 4 * t1 + t3
    t5 = 4 * t0 + t2
    return (t3 + t5) % R, t5 % R, (t2 + t4) % R, t4 % R


def permute(s):
    """bb poseidon2_permutation on a 4-element state (list/tuple of ints mod R). Returns a new list."""
    a, b, c, d = s
    # initial external linear layer M4
    t0 = a + b; t1 = c + d; t2 = b + b + t1; t3 = d + d + t0  # noqa: E702
    t4 = 4 * t1 + t3; t5 = 4 * t0 + t2  # noqa: E702
    a, b, c, d = (t3 + t5) % R, t5 % R, (t2 + t4) % R, t4 % R
    e = EXT
    for r in (0, 1, 2, 3, None, 4, 5, 6, 7):
        if r is None:
            d0, d1, d2, d3 = DIAG
            for rc in INT:
                x = (a + rc) % R
                ...
                a = x2 * x2 % R * x % R
                t = a + b + c + d
                a, b, c, d = (a * d0 + t) % R, (b * d1 + t) % R, (c * d2 + t) % R, (d * d3 + t) % R
            continue
        k = 4 * r
        x = (a + e[k]) % R; x2 = x * x % R; a = x2 * x2 % R * x  # noqa: E702
        x = (b + e[k + 1]) % R; x2 = x * x % R; b = x2 * x2 % R * x  # noqa: E702
        x = (c + e[k + 2]) % R; x2 = x * x % R; c = x2 * x2 % R * x  # noqa: E702
        x = (d + e[k + 3]) % R; x2 = x * x % R; d = x2 * x2 % R * x  # noqa: E702
        t0 = a + b; t1 = c + d; t2 = b + b + t1; t3 = d + d + t0  # noqa: E702
        t4 = 4 * t1 + t3; t5 = 4 * t0 + t2  # noqa: E702
        a, b, c, d = (t3 + t5) % R, t5 % R, (t2 + t4) % R, t4 % R
    return [a, b, c, d]
...
# ---- oalib domain tags (capacity cell s[3])
TAG_LEAF = 262145  # 1 + (1024 << 8)
TAG_NODE = 2
TAG_HALF = 8
TAG_FS = 101897
TAG_CODE = 10
L = 12000  # template length (48 kHz)
LEAF = 1024  # samples per leaf
NLEAVES = 256  # 4^4, depth 4
CAP = NLEAVES * LEAF  # 262,144 samples


class Sp:
    """oalib sponge: rate 3, s[3] = tag, absorb adds into s[pos], permute when 3 absorbed;
    finish permutes only if pos != 0 (a length that is a multiple of 3 ends without an extra permutation,
    and an empty input returns the tag state's s[0] = 0 unpermuted). Returns (s[0], s[1])."""
    ...
    def absorb(self, x: int) -> None:
        self.s[self.pos] = (self.s[self.pos] + x) % R
        self.pos += 1
        if self.pos == 3:
            self.s = permute(self.s)
            self.pos = 0
...
def pack_leaf(u) -> list[int]:
    """1,024 u16 values (u = sample + 32768) -> 69 elements of 15 x 16 bits, little-end within an element."""
    assert len(u) == LEAF
    out = []
    for k in range(69):
        acc = 0
        for j, v in enumerate(u[15 * k: 15 * k + 15]):
            acc |= int(v) << (16 * j)
        out.append(acc)
    return out


def leaf_hash_u(u) -> int:
    """leaf over 1,024 offset samples u in [0, 65535]."""
    return hash_n(TAG_LEAF, pack_leaf(u))


def leaf_hash(x) -> int:
    """leaf over 1,024 signed int16 samples."""
    return leaf_hash_u([int(v) + 32768 for v in x])


def node_hash(c) -> int:
    assert len(c) == 4
    return hash_n(TAG_NODE, c)


def rec_levels(x) -> list[list[int]]:
    """rec tree over a capture of signed int16 samples (len <= 262,144), zero-padded to 256 leaves
    exactly as gen_inputs.py (padding sample 0 -> u = 32768; every leaf is hashed, no null nodes).
    Returns [256 leaves, 64, 16, 4, 1 root]."""
    x = [int(v) for v in x]
    assert len(x) <= CAP, len(x)
    full = x + [0] * (CAP - len(x))
    zero_leaf = None
    lv = []
    for i in range(NLEAVES):
        blk = full[i * LEAF:(i + 1) * LEAF]
        if (i + 1) * LEAF > len(x) and not any(blk):
            if zero_leaf is None:
                zero_leaf = leaf_hash(blk)
            lv.append(zero_leaf)
        else:
            lv.append(leaf_hash(blk))
    levels = [lv]
    while len(levels[-1]) > 1:
        c = levels[-1]
        levels.append([node_hash(c[4 * j: 4 * j + 4]) for j in range(len(c) // 4)])
    return levels


def rec_root(x) -> int:
    return rec_levels(x)[-1][0]


def rec_path(levels, idx: int) -> list[list[int]]:
    """the 4 sibling groups (chs/chp row) of leaf idx, bottom up, as gen_inputs.path()."""
    out = []
    for l in range(4):
        b = (idx // 4) * 4
        out.append(levels[l][b: b + 4])
        idx //= 4
    return out


def _absorb_bytes(sp: Sp, a) -> None:
    # 31 bytes per element, big-endian; last element holds the L % 31 remaining bytes
    for k in range(0, len(a), 31):
        sp.absorb(int.from_bytes(bytes(a[k:k + 31]), "big"))


def code_commitment(c_is, c_qs, c_ip, c_qp) -> int:
    """over the four int8 templates (values -128..127, stored as u8 = v + 128), L each."""
    sp = Sp(TAG_CODE)
    for t in (c_is, c_qs, c_ip, c_qp):
        assert len(t) == L
        _absorb_bytes(sp, [int(v) + 128 for v in t])
    return sp.finish()[0]


def code_commitment_u8(a, b, c, d) -> int:
    """same, templates already as u8 (v + 128) bytes."""
    sp = Sp(TAG_CODE)
    for t in (a, b, c, d):
        assert len(t) == L
        _absorb_bytes(sp, t)
...
def half_commit(nonce_hi, nonce_lo, attempt, role_b, sr, half, salt, x_own, x_partner) -> int:
    """hash_n(TAG_HALF, 11 inputs). half is signed (mod R); x_own / x_partner are 32-byte X coordinates
    (bytes) or ready [hi128, lo128] limbs."""
    def limbs(x):
        if isinstance(x, (bytes, bytearray)):
            assert len(x) == 32
            return [int.from_bytes(x[:16], "big"), int.from_bytes(x[16:], "big")]
        return [int(x[0]), int(x[1])]
    xo, xp = limbs(x_own), limbs(x_partner)
    return hash_n(TAG_HALF, [nonce_hi, nonce_lo, attempt, 1 if role_b else 0, sr, int(half) % R, salt,
                             xo[0], xo[1], xp[0], xp[1]])


# ---- holder_commit: oalib has no definition. Two conventions exist in the prototypes; not picked here.
def holder_commit_opt3(secret: int) -> int:
    """opt3 prototype (opt3-noir-p256 hashhelper / poplib): noir-lang/poseidon v0.3.0
    Poseidon2::hash([secret], 1) = sponge with IV = 1 << 64 in s[3], s[0] += secret, one permutation."""
    return hash_n(1 << 64, [secret])


def holder_commit_sp(secret: int, tag: int) -> int:
    """oalib-Sp style: hash_n(tag, [secret]). No holder tag is defined in oalib; the caller must choose one."""
    return hash_n(tag, [secret])


def poseidon2_hash(xs) -> int:
    """noir-lang/poseidon v0.3.0 Poseidon2::hash(xs, len(xs)) (IV = len << 64)."""
    xs = list(xs)
    if not xs:
        return permute([0, 0, 0, 0])[0]
    return hash_n(len(xs) << 64, xs)


def to_bytes32(v: int) -> bytes:
