//! oa-vega-rx (review copy: + int16 range on x): the JBL250 arrival statement (same as ../gen_circuit.py) on Spartan2/Vega
...
impl VegaCircuit<E> for Arrival {
    ...
    fn synthesize<CS: ConstraintSystem<F>>(
        ...
        // REVIEW: int16 range check on every window sample (the circom version has it in Commit16)
        if std::env::var("OA_NO_XRANGE").is_err() {
            for xi in &x {
                range(cs, &xi.add(&Lc::cnst(F::from(32768u64))), 16)?;
            }
        }
        ...
        // REVIEW fix (opt-in): arr <= K-1-LOOK, so the half-rule lookahead is never cut off by the window end
        if std::env::var("OA_LOOK_FIX").is_ok() {
            enforce_eq_prod(cs, &sel[k - 1 - LOOK], &one(), &Lc::zero());
        }
        for j in 0..k {
