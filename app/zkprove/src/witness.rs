...
/// `inputs`: Prover.toml text, or the same map as a JSON object (starts with '{').
pub fn solve(artifact_json: &[u8], inputs: &str) -> Res<Vec<u8>> {
    let art: ProgramArtifact = serde_json::from_slice(artifact_json).map_err(|e| format!("artifact: {e}"))?;
    let fmt = if inputs.trim_start().starts_with('{') { Format::Json } else { Format::Toml };
    let inputs = fmt.parse(inputs, &art.abi).map_err(|e| format!("inputs: {e}"))?;
